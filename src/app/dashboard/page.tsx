...
import { getDb } from '@/lib/db/localStore'
...
const FutureSimulator = dynamic(() => import('@/components/dashboard/future-simulator').then(mod => mod.FutureSimulator))
const GithubContributionGrid = dynamic(() => import('@/components/dashboard/github-contribution-grid').then(mod => mod.GithubContributionGrid))
...
export default async function DashboardPage() {
  ...
  const totalImpact = activities.reduce((sum, act) => sum + Number(act.carbon_impact_kg), 0)

  // Calculate category impacts for Concentric Rings
  const transportImpact = activities
    .filter((a) => a.category === 'transport')
    .reduce((sum, act) => sum + Number(act.carbon_impact_kg), 0)
  ...
  const energyImpact = activities
    .filter((a) => a.category === 'electricity')
    .reduce((sum, act) => sum + Number(act.carbon_impact_kg), 0)
  ...
  const carbonScore =
    activities.length === 0 ? 1000 : Math.max(0, 1000 - Math.floor(totalImpact * 5))
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground flex-1 w-full border-t border-border/20 transition-colors duration-300">
      ...
      <div className="absolute inset-0 bg-[linear-gradient(to_right,var(--border)_1px,transparent_1px),linear-gradient(to_bottom,var(--border)_1px,transparent_1px)] bg-[size:4rem_4rem] opacity-[0.08] pointer-events-none" />
      <div className="absolute top-0 left-1/4 -z-10 w-96 h-96 bg-[var(--dashboard-glow,rgba(16,185,129,0.06))] rounded-full blur-[120px] opacity-60 transition-all duration-500 pointer-events-none" />
      <div className="absolute bottom-10 right-1/4 -z-10 w-[450px] h-[450px] bg-teal-500/5 rounded-full blur-[140px] opacity-40 pointer-events-none" />
      ...
      <div className="container mx-auto py-8 px-4 max-w-7xl relative z-10 space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-border">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Here is your digital twin footprint and impact center.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              className={cn(
                buttonVariants({
                  className:
                    'bg-primary hover:bg-primary/95 text-primary-foreground font-extrabold h-10 px-5 rounded-xl transition-all shadow-md hover:shadow-lg cursor-pointer',
                }),
              )}
            >
        ...
        <Tabs defaultValue="overview" className="space-y-8">
          <TabsList className="flex flex-nowrap overflow-x-auto scrollbar-none gap-1">
            <TabsTrigger
              value="overview"
              className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground font-semibold px-4 py-3.5 rounded-lg text-sm transition-all cursor-pointer shrink-0"
            >
              Overview
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="overview" className="space-y-8 outline-none">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 items-stretch">
              <Card className="bg-card border-border rounded-xl shadow-sm relative overflow-hidden flex flex-col justify-between h-full min-h-[140px] hover:shadow-md hover:border-primary/20 transition-all duration-300">
                <div className="absolute top-0 right-0 -mr-8 -mt-8 w-16 h-16 bg-primary/10 rounded-full blur-xl" />
                <CardHeader className="p-6">
                  <CardDescription className="text-muted-foreground font-semibold text-xs uppercase tracking-wider">
                    Total Footprint
                  </CardDescription>
                  <CardTitle className="text-3xl font-bold text-primary mt-2 flex items-baseline gap-2">
                    {totalImpact.toFixed(1)}{' '}
                    <span className="text-sm font-semibold text-muted-foreground">kg CO₂e</span>
                  </CardTitle>
                </CardHeader>
              </Card>
            ...
            <div className="grid gap-6 lg:grid-cols-3">
              ...
              <div className="lg:col-span-2 flex flex-col space-y-6">
                ...
                <Card className="bg-card border-border rounded-xl shadow-sm relative overflow-hidden flex-1">
                  <div className="absolute top-0 left-0 -ml-16 -mt-16 w-28 h-28 bg-primary/5 rounded-full blur-2xl" />
                  <CardHeader className="pb-3 border-b border-border/30">
                    ...
                    <CardDescription className="text-xs">
                      Personalized lifestyle recommendations from Llama-3.1
                    </CardDescription>
                  </CardHeader>
                      ...
                      {insights.length > 0 ? (
                        ...
                        <div className="p-8 rounded-xl border border-dashed border-border text-center">
                          <p className="text-xs text-muted-foreground">
                            Log some activities to synchronize and generate AI Twin insights!
                          </p>
                        </div>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Visualize your carbon footprint breakdown and comparative grid generation
                      stats.
                    </p>
                  </div>
                </div>
